...
/* global addGalaxyShells, addGalaxyStars, addSollySun, addPlanets, addSollys, addWhiteStars, addSolly1AndSolly2 */
...
class Level2Manager {
  ...
  createShapeChoicesPanel() {
    ...
    const currentShape =
      globalWindow.gameManager && globalWindow.gameManager.getCurrentShape
        ? globalWindow.gameManager.getCurrentShape()
        : 'piramide';
  ...
  placeShapeOnCorner(shapeChoice, corner) {
    try {
      ...
      console.log(
        `✅ Shape geplaatst op corner ${corner.userData.cornerIndex}. ${this.placedShapes}/${this.totalCorners} voltooid`,
      );
    } catch (error) {
    ...
    const animateWhirl = (/** @type {number} */ currentTime) => {
      ...
      if (progress < 1) {
        ...
        if (currentTime - this.lastFrameTime > 16) {
          // ~60fps
          this.wireframeCube.children.forEach(
  ...
  restartMainGame() {
    ...
    try {
      ...
      // Solly1 al aanwezig vanuit main.js - niet opnieuw toevoegen
