...
class GameManager {
  ...
  async loadDefaultConfig() {
    return window.errorHandler.safeExecuteAsync(
      async () => {
      ...
      'loadDefaultConfig',
      window.errorHandler.getDefaultConfig(),
    );
  }
  ...
  loadCoinData(coinData) {
    ...
    this.currentUserData = {
      ...
      size:
        coinData.size || this.getDefaultSizeForLevel(coinData.level || this.defaultConfig.level),
  ...
  getDefaultSizeForLevel(level) {
    switch (level) {
      case 'beginner':
        return 200; // 200% - Makkelijkste
      case 'level1':
        return 175; // 175% - Nog steeds vrij groot
      case 'level2':
        return 150; // 150% - Medium grootte
      case 'level3':
        return 100; // 100% - Normale grootte, moeilijkst
      case 'master':
        return 200; // 200% - Makkelijkste (met extra features)
      default:
        return 200; // Fallback naar makkelijkste
    }
  }
  ...
  incrementKaboomCount(level = 1, position = null, shape = null) {
    if (this.currentUserData) {
      ...
      const dbCount =
        this.databaseManager && this.databaseManager.isInitialized
